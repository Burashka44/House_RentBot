...
async def normalize_address_enhanced(raw_address: str) -> NormalizedAddress:
    """
    Enhanced address normalization using DaData (if available) with regex fallback.
    ...
    if dadata:
        try:
            ...
            if house_info and house_info.get("house"):
                # Successfully got normalized address from DaData
                return NormalizedAddress(
                    city=house_info.get("city") or "Неизвестно",
                    street=house_info.get("street") or "Неизвестно",
                    house_number=house_info.get("house") or "0",
                    region=house_info.get("region")
                )
        except Exception as e:
            logging.warning(f"DaData normalization failed, using fallback: {e}")
