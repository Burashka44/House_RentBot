...
async def tenant_menu(message: Message, tenant):
...
async def on_photo_received(message: Message, tenant, state: FSMContext, session: AsyncSession):
    ...
    if current_state == ReceiptState.waiting_for_photo.state:
        await _process_receipt_impl(message, tenant, file_id, state, session)
        return

    # 2. Ambiguous -> Ask user
    await state.update_data(temp_file_id=file_id, temp_caption=message.caption)
...
async def on_receipt_confirmed(call: CallbackQuery, tenant, state: FSMContext, session: AsyncSession):
    ...
    await _process_receipt_impl(call.message, tenant, file_id, state, session)
...
async def _process_receipt_impl(message: Message, tenant, file_id: str, state: FSMContext, session: AsyncSession):
    ...
    if decision == ReceiptDecision.accepted:
        ...
        text += UIMessages.field("Статус", "Ожидает проверки", UIEmojis.PENDING)
        text += "\n" + UIMessages.info_box("Администратор проверит платёж в ближайшее время")
        await message.answer(text)
        ...
        text += "\n" + UIMessages.info_box("Пожалуйста, отправьте более чёткое фото или проверьте правильность чека")
        await message.answer(text)
...
async def back_to_tenant_menu(call: CallbackQuery, tenant):
    from bot.utils.ui import UIEmojis, UIMessages
    from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
...
@router.message(F.text.contains("Мои начисления"))
...
async def settings_menu(message: Message, tenant, session: AsyncSession):
    from bot.utils.ui import UIEmojis, UIMessages
...
# --- My Object ---
@router.message(F.text.contains("Мой объект"))
